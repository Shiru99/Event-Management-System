...
class InvigilatorsForEvent extends StatefulWidget {
  PostgresKonnection _postgresKonnection;
  // ignore: non_constant_identifier_names
  String event_id;
  SaveUser _user;
  InvigilatorsForEvent(this._user, this._postgresKonnection, this.event_id);

  @override
  _InvigilatorsForEventState createState() => _InvigilatorsForEventState();
}

class _InvigilatorsForEventState extends State<InvigilatorsForEvent> {
  var results;
...
Widget AdminInvigilatorPostUI(
  ...
  Future removeThisInviligator() async {
    toastMessage("Removing...");
    PostgreSQLConnection _konnection =
    ...
    try {
      await _konnection.transaction((ctx) async {
        await ctx.query('''
          ...
          DELETE FROM appoint WHERE invigilator_id=@a AND event_id=@b
            ''', substitutionValues: {
          "a": guest_id,
          "b": event_id,
        });
      ...
      print(" removed invigilator done");
